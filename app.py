...
# 2. الإعدادات
api_key = st.text_input("مفتاح Gemini API (مطلوب)", type="password", help="المفتاح مجاني من Google AI Studio")
...
if st.button("توكلنا على الله.. ابدأ التحويل 🚀"):
    ...
    elif not api_key: st.warning("⚠️ مفتاح API مفقود.")
...
# ---------------------------------------------------------
# صندوق الملاحظات (الصدقة الجارية) - التعديل الجديد 🆕
# ---------------------------------------------------------
with st.sidebar:
    st.header("💌 رسالة ودعوة")
    # استرجاع نص الصدقة الجارية والدعاء مع الحفاظ على تنسيق RTL
    st.markdown("""
