    ...
    with st.form(key='feedback_form'):
        ...
        if submit_feedback:
            if feedback_text:
                ...
                st.warning("الرجاء كتابة رسالة أولاً.")import streamlit as st
import google.generativeai as genai
from docx import Document
from docx.shared import Pt, RGBColor
from docx.enum.text import WD_ALIGN_PARAGRAPH
...
# ---------------------------------------------------------
# CSS للمظهر (تحسين الـ RTL + ألوان MedMate)
# ---------------------------------------------------------
st.markdown("""
<style>
/* 1. ضبط اتجاه الصفحة بالكامل لليمين */
...
/* 4. تعديل مدخلات النصوص (Inputs) */
.stTextInput input, .stTextArea textarea {
    direction: rtl;
    text-align: right;
}

/* تعديل محاذاة الـ Checkbox و Radio Buttons */
.stCheckbox, .stRadio {
    direction: rtl;
    text-align: right;
}
...
# 2. جلب مفتاح Gemini API (الآن من الأسرار فقط)
try:
    api_key = st.secrets["GEMINI_API_KEY"]
except:
    api_key = None # سيعطي تنبيهاً لاحقاً إذا لم يتم العثور عليه
...
# 1. منطقة الرفع
st.info("📂 ارفع صور المحاضرات (سبورة/ورق) أو ملفات PDF")
uploaded_files = st.file_uploader(
    "", 
    type=['png', 'jpg', 'jpeg', 'pdf'], 
    accept_multiple_files=True
...
# 2. الإعدادات (تم حذف خانة الـ API Key من هنا) ❌

doc_type_selection = st.radio(
    "نوع المحتوى (Output Format):",
    options=["Lecture / Notes", "Exam / MCQ"],
    captions=[
        "للمحاضرات والمذكرات: تنسيق كفقرات وعناوين وشرح.",
        "للامتحانات: تنسيق كأسئلة منفصلة واختيارات دقيقة."
    ],
    index=None,
    horizontal=True
)
...
if st.button("توكلنا على الله.. ابدأ التحويل 🚀"):
    if not uploaded_files: st.warning("⚠️ الرجاء رفع الملفات أولاً.")
    elif not api_key: st.error("⚠️ لم يتم العثور على مفتاح API في الإعدادات! يرجى التواصل مع المطور.") # رسالة تظهر فقط لو نسيت وضع المفتاح في الأسرار
    elif doc_type_selection is None: st.error("🛑 يجب اختيار نوع المحتوى لضمان جودة الملف.")
    else:
        genai.configure(api_key=api_key)
        model = genai.GenerativeModel('gemini-flash-latest')
